...
table(adams$Manuscript_Identity[which(adams$Manuscript_Identity == "T")])
...
covariates <- c("Subclass_Cell_Identity",
...
for(covariate in covariates){
  plot1 <-Seurat::DimPlot(adams, reduction = "umap",
                          ...
                          raster = F)
  plot1 <- plot1 + ggplot2::ggtitle(paste0("Adams (T-cells): ", covariate))
  plot1 <- plot1 + ggplot2::theme(legend.text = ggplot2::element_text(size = 5))
  width <- ifelse(covariate %in% c("Subclass_Cell_Identity", "Subject_Identity"), 7, 5)
  ggplot2::ggsave(filename = paste0("../../../../out/fig/Writeup11b/Writeup11b_adams_exploration_umap_T_", covariate, ".png"),
...
save(adams, date_of_run, session_info,
     file = "../../../../out/Writeup11b/Writeup11b_adams_T_preprocessed.RData")
