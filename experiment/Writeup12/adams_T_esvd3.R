...
covariate_dat <- adams@meta.data[,c("Disease_Identity", "Subject_Identity", "Gender",
                                    "Tobacco","percent.mt", "Age", "Ethnicity")]
...
covariate_df[,"Subject_Identity"] <- factor(covariate_df[,"Subject_Identity"], levels = names(sort(table(covariate_df[,"Subject_Identity"]), decreasing = T)))
...
save(date_of_run, session_info, adams,
     ...
     file = "../../../../out/Writeup12/adams_T_esvd3.RData")
