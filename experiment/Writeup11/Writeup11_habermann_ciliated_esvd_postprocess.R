...
case_idx <- which(metadata[,"Diagnosis"] == "IPF")
...
individual_stats <- lapply(1:ncol(mat), function(j){
  ...
  case_gaussians <- sapply(case_individuals, function(indiv){
    cell_names <- rownames(metadata)[which(metadata$Sample_Name == indiv)]
...
png("../../../../out/fig/Writeup11/habermann_ciliated_esvd_teststat_histogram.png", height = 1200, width = 1200,
    units = "px", res = 300)
...
png("../../../../out/fig/Writeup11/habermann_ciliated_esvd_teststat_histogram_separate.png",
...
save(teststat_vec, session_info, date_of_run,
     file = "../../../../out/Writeup11/Writeup11_habermann_ciliated_esvd_teststat.RData")

teststat_vec_habermann <- teststat_vec
load("../../../../out/Writeup11/Writeup11_adams_ciliated_esvd_teststat.RData")
teststat_vec_adams <- teststat_vec

cor_val <- stats::cor(teststat_vec_adams, teststat_vec_habermann,
                      method = "spearman")
idx <- unique(c(other_idx, hk_idx, de_idx))
png("../../../../out/fig/Writeup11/Writeup11_lung_ciliated_adams_habermann.png",
    width = 2500, height = 2500, res = 300, units = "px")
plot(NA, xlim = range(teststat_vec_habermann), ylim = range(teststat_vec_adams),
     xlab = "Habermann test statistic", ylab = "Adams test statistic",
     main = paste0("Ciliated cells,\nSpearman correlation: ", round(cor_val,2)))
points(x = teststat_vec_habermann[-idx], y = teststat_vec_adams[-idx],
       col = col_vec[-idx], pch = 16)
points(x = teststat_vec_habermann[idx], y = teststat_vec_adams[idx],
       col = col_vec[idx], pch = 16)
graphics.off()
