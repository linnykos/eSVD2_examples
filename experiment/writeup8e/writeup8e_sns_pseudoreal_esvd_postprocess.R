rm(list=ls())
load("../../../../out/writeup8e/writeup8e_sns_pseudoreal_esvd_poisson.RData")
load("../../../../out/writeup8e/writeup8e_sns_pseudoreal_data.RData")
de_idx <- autism_gene_idx
...
### let's draw it nicer
png("../../../../out/fig/writeup8e/sns_pseudoreal_volcano.png", height = 1200, width = 1200,
    units = "px", res = 300)
plot(NA, xlim = c(-3,3), ylim = range(0, 20), bty = "n",
...
png("../../../../out/fig/writeup8e/sns_pseudoreal_zscore_histogram.png", height = 1200, width = 1200,
    units = "px", res = 300)
hist(zz, breaks = seq(-10.05, 10.05, by = 0.1), xlim = c(-10,10),
...
load("../../../../out/writeup8e/writeup8e_sns_pseudoreal_data.RData")
cor(log(nuisance_param_vec_est), log(nuisance_param_vec))
cor(true_esvd$b_mat[,"Log_UMI"], esvd_res_full$b_mat[,"Log_UMI"])
cor(true_esvd$b_mat[,"diagnosis_ASD"], esvd_res_full$b_mat[,"diagnosis_ASD"])
cor(true_esvd$b_mat[,"Intercept"], esvd_res_full$b_mat[,"Intercept"])

true_mean <- library_mat*lambda_mat
nat_mat1 <- tcrossprod(esvd_res_full$x_mat, esvd_res_full$y_mat)
nat_mat2 <- tcrossprod(esvd_res_full$covariates, esvd_res_full$b_mat)
nat_mat <- nat_mat1 + nat_mat2
est_mean <- exp(nat_mat)
idx <- sample(1:prod(dim(true_mean)), 1e6)
cor(true_mean[idx], est_mean[idx])
