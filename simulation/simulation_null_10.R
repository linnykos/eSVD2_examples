...
x_mat <- cbind(runif(n, min = 0, max = 1),
               runif(n, min = 0, max = 1))
...
# form nuisance
set.seed(10)
dispersion_vec <- sample(rep(c(10, 1, 0.01), each = ceiling(p/3))[1:p])
dispersion_vec[1:10] <- 10
dispersion_vec[(round(p*.7)+1):p] <- 0.01
...
size_noise <- runif(length(case_df_idx), min = -max_add, max = max_add)

for(j in 1:10){
  for(kk in 1:length(case_df_idx)){
    cell_idx <- cell_individual_list[[case_df_idx[kk]]]
    tmp <- nat_mat[cell_idx,j]
    nat_mat[cell_idx,j] <- tmp+size_noise[kk]
  }

  for(kk in 1:length(control_df_idx)){
    cell_idx <- cell_individual_list[[control_df_idx[kk]]]
    tmp <- nat_mat[cell_idx,j]
    nat_mat[cell_idx,j] <- tmp-size_noise[kk]
  }
}
...
set.seed(10)
available_idx <- intersect(11:p, which(dispersion_vec == min(dispersion_vec)))
null_downsample_idx <- sample(available_idx, round(0.1*length(available_idx)))
for(j in null_downsample_idx){
  cell_idx <- sample(1:nrow(obs_mat), round(nrow(obs_mat)*.4))
...
save(seurat_obj,
     ...
     file = "../eSVD2_examples/simulation/simulation_null_10.RData")
