...
load("../../../../out/Writeup12/habermann_T_esvd3.RData")
eSVD_obj_habermann <- eSVD_obj

load("../../../../out/Writeup12/adams_T_esvd3.RData")
...
png(paste0("../../../../out/fig/Writeup12/adams_habermann_T_esvd3_teststatistic.png"),
...
png("../../../../out/fig/Writeup12/adams_habermann_T_esvd3-agreement_de-genes.png",
...
y1_med <- median(eSVD_obj_adams$teststat_vec)
y2_med <- median(eSVD_obj_habermann$teststat_vec)
tab <- table(y1 >= y1_med, y2 >= y2_med); (tab[1,1]+tab[2,2])/sum(tab)
...
locfdr_res <- locfdr::locfdr(gaussian_teststat_adams, plot = 0)
