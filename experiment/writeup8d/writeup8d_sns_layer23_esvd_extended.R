...
# zz <- tcrossprod(init_x_mat, init_y_mat)
# svd_res <- irlba::irlba(zz, nv = ncol(init_x_mat))
# round(svd_res$d, 4)
zz <- tmp$residual_mat
...
gene_group_factor <- rep(2, ncol(mat))
nuisance_param_vec_full <- rep(0.5, ncol(mat))
gene_group_factor[gene_idx] <- 2+(1:length(gene_idx))
...
nuisance_value_lower[gene_idx] <- 1
...
esvd_res_nb2 <- eSVD2::opt_esvd(esvd_res_nb1$x_mat,
                                ...
                                gene_group_factor = factor(1:ncol(mat)),
                                nuisance_value_lower = nuisance_value_lower,
                                nuisance_value_upper = nuisance_value_upper,
                                reestimate_nuisance = T,
                                reestimate_nuisance_per_iteration = 10,
