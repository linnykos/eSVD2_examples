...
mast_pthres <- min(mast_logpvalue_vec[mast_selected_genes])
...
mast_idx <- which(names(y_vec) %in% mast_selected_genes)
...
png("../../../out/fig/main/sns_layer23_cross-comparison_mast.png",
...
lines(y = c(-1e4,1e4), x = rep(mast_pthres, 2), col = "white", lty = 2, lwd = 4)
lines(x = c(-1e4,1e4), y = rep(esvd_pthres, 2), col = orange_col, lty = 2, lwd = 2)
lines(y = c(-1e4,1e4), x = rep(mast_pthres, 2), col = yellow_col, lty = 2, lwd = 2)
...
points(x = x_vec[mast_idx], y = y_vec[mast_idx],
       col = yellow_col, pch = 16, cex = 1.5)

# plot non-overlapping genes
points(x = x_vec[setdiff(bulk_idx, c(esvd_idx, mast_idx))], y = y_vec[setdiff(bulk_idx, c(esvd_idx, mast_idx))],
       col = blue_col, pch = 16, cex = 1, lwd = 2)
points(x = x_vec[setdiff(sfari_idx, c(esvd_idx, mast_idx))], y = y_vec[setdiff(sfari_idx, c(esvd_idx, mast_idx))],
...
points(x = x_vec[intersect(hk_idx, c(esvd_idx, mast_idx))], y = y_vec[intersect(hk_idx, c(esvd_idx, mast_idx))],
       col = green_col, pch = 16, cex = 1)

# circle overlapping gens
points(x = x_vec[intersect(c(esvd_idx, mast_idx), sfari_idx)], y = y_vec[intersect(c(esvd_idx, mast_idx), sfari_idx)],
       col = "white", pch = 1, cex = 2, lwd = 3)
points(x = x_vec[intersect(c(esvd_idx, mast_idx), sfari_idx)], y = y_vec[intersect(c(esvd_idx, mast_idx), sfari_idx)],
       col = purple_col, pch = 1, cex = 2, lwd = 2)
points(x = x_vec[intersect(c(esvd_idx, mast_idx), bulk_idx)], y = y_vec[intersect(c(esvd_idx, mast_idx), bulk_idx)],
       col = "white", pch = 1, cex = 2, lwd = 3)
points(x = x_vec[intersect(c(esvd_idx, mast_idx), bulk_idx)], y = y_vec[intersect(c(esvd_idx, mast_idx), bulk_idx)],
