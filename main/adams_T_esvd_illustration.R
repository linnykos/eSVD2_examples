...
table(adams$Tobacco, adams$Disease_Identity)
adams[["pca"]] <- NULL
adams[["umap"]] <- NULL
...
set.seed(10)
adams <- Seurat::RunUMAP(adams, dims = 1:50, spread = 0.75, min.dist = 0.1)

umap_mat <- adams[["umap"]]@cell.embeddings
n <- nrow(umap_mat)
color_vec <- rep(NA, n)
for(i in 1:length(case_subj)){
  color_vec[adams$Subject_Identity == case_subj[i]] <- case_color_palette[i]
}
for(i in 1:length(control_subj)){
  color_vec[adams$Subject_Identity == control_subj[i]] <- control_color_palette[i]
}
png("../../../out/fig/main/adams_T_umap_prior_subject.png",
    height = 3000, width = 3000,
    units = "px", res = 500)
...
png("../../../out/fig/main/adams_T_umap_prior_smoking.png",
...
png("../../../out/fig/main/adams_T_umap_prior_gender.png",
    height = 1500, width = 1500,
    units = "px", res = 500)
par(mar = c(3,3,0.1,0.1))
plot(umap_mat[,1], umap_mat[,2], col = color_vec,
...
mt_range <- range(adams$percent.mt)
mt_breaks <- seq(mt_range[1], mt_range[2], length.out = 100)
color_vec <- sapply(1:n, function(i){
  color_palette[which.min(abs(adams$percent.mt[i] - mt_breaks))]
})
png("../../../out/fig/main/adams_T_umap_prior_percentmt.png",
    height = 1500, width = 1500,
    units = "px", res = 500)
par(mar = c(3,3,0.1,0.1))
plot(umap_mat[,1], umap_mat[,2], col = color_vec,
...
png("../../../out/fig/main/adams_T_umap_prior_logumi.png",
    height = 1500, width = 1500,
    units = "px", res = 500)
par(mar = c(3,3,0.1,0.1))
plot(umap_mat[,1], umap_mat[,2], col = color_vec,
...
png("../../../out/fig/main/adams_T_umap_after_subject.png",
    height = 3000, width = 3000,
    units = "px", res = 500)
par(mar = c(3,3,0.1,0.1))
...
color_vec[adams$Gender == "Female"] <- rgb(235, 134, 47, maxColorValue = 255)
color_vec[adams$Gender == "Male"] <- rgb(184, 54, 220, maxColorValue = 255)
png("../../../out/fig/main/adams_T_umap_after_gender.png",
    height = 1500, width = 1500,
    units = "px", res = 500)
par(mar = c(3,3,0.1,0.1))
plot(umap_mat[idx,1], umap_mat[idx,2], col = color_vec[idx],
...
color_vec[adams$Tobacco == "Yes"] <- rgb(214, 55, 55, maxColorValue = 255)
color_vec[adams$Tobacco == "No"] <- rgb(146, 146, 146, maxColorValue = 255)
png("../../../out/fig/main/adams_T_umap_after_smoking.png",
    height = 1500, width = 1500,
    units = "px", res = 500)
par(mar = c(3,3,0.1,0.1))
plot(umap_mat[idx,1], umap_mat[idx,2], col = color_vec[idx],
...
zz = cbind(round(y_vec[adams_df_genes],2), round(x_vec[adams_df_genes],2))
