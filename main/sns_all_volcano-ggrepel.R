...
for(kk in 1:length(celltypes)){
  ...
  selected_genes <- sort(esvd_selected_genes)
  ...
  plot1 <- ggplot2::ggplot(df, ggplot2::aes(x = lfc, y = esvd_log10pvalue))
  ...
  ggplot2::ggsave(filename = paste0("../../../out/fig/main/sns_", celltype, "_volcano_ggrepel.png"),
                  plot1, device = "png", width = 5*.75, height = 7*.75, units = "in")

  if(celltype == "layer23"){
    ggplot2::ggsave(filename = paste0("../../../out/fig/main/sns_", celltype, "_volcano_ggrepel_smaller.png"),
