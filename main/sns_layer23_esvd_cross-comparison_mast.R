...
# minor adjustments
mast_logpvalue_vec <- mast_logpvalue_vec[names(esvd_logpvalue_vec)]
...
selected_genes <- sort(unique(c(mast_selected_genes, esvd_selected_genes)))
...
col_vec[mast_selected_genes] <- yellow_col
col_vec[intersect(mast_selected_genes, esvd_selected_genes)] <- red_col
...
df <- data.frame(
  ...
  mast_log10pvalue = mast_logpvalue_vec,
  bulk = bulk_vec,
  col = col_vec,
  circled = circle_vec,
  hk = hk_vec,
...
plot1 <- ggplot2::ggplot(df, ggplot2::aes(x = mast_log10pvalue, y = esvd_log10pvalue))
plot1 <- plot1 + ggplot2::geom_hline(yintercept = esvd_pthres, color = "white", linewidth = 2)
plot1 <- plot1 + ggplot2::geom_vline(xintercept = mast_pthres, color = "white", linewidth = 2)
plot1 <- plot1 + ggplot2::geom_hline(yintercept = esvd_pthres, linetype = "dashed", color = orange_col, linewidth = 1.5)
plot1 <- plot1 + ggplot2::geom_vline(xintercept = mast_pthres, linetype = "dashed", color = yellow_col, linewidth = 1.5)
...
plot1 <- plot1 + ggplot2::labs(x = "", y = "", title = paste0("Correlation: ", round(stats::cor(esvd_logpvalue_vec, mast_logpvalue_vec), 2)))
...
ggplot2::ggsave(filename = paste0("../../../out/fig/main/sns_layer23_volcano_cross-comparison_mast_ggrepel.png"),
