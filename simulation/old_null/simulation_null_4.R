...
x_mat <- cbind(runif(n, min = -1, max = 0.5),
               runif(n, min = -1, max = 0.5))
y_mat <- cbind(runif(p, min = 0, max = 2),
               runif(p, min = 0, max = 2))
...
z_mat <- cbind(rep(-1, p), # intercept
               rep(1, p), # library
               c(rep(2,10),rep(0, p-10)), # cc
               rnorm(p, sd = 1), # sex
               rnorm(p, sd = 1)) # age
...
save(seurat_obj,
     ...
     file = "../eSVD2_examples/simulation/simulation_null_4.RData")
