...
load("../../../../out/Writeup11g/Writeup11g_sns_layer23_esvd.RData")
...
cc_idx <- which(names(eSVD_obj$teststat_vec) %in% cycling_genes)
...
other_idx <- setdiff(other_idx, de_idx)
cc_idx <- setdiff(cc_idx, c(de_idx, other_idx))
hk_idx <- setdiff(hk_idx, c(de_idx, other_idx, cc_idx))
...
col_vec[de_idx] <- 2
col_vec[cc_idx] <- 5
shuf_idx <- c(hk_idx, de_idx, other_idx, cc_idx)
...
png("../../../../out/fig/Writeup11g/Writeup11g_sns_layer23_esvd_teststat_histogram.png",
...
legend("topright", c("Published DE gene", "Other interest gene", "Housekeeping gene", "Cell-cycle gene"),
       fill = c(2,4,3,5), cex = 0.6)
graphics.off()

max_val <- max(abs(eSVD_obj$teststat_vec[shuf_idx]))
png("../../../../out/fig/Writeup11g/Writeup11g_sns_layer23_esvd_teststat_histogram_separate.png",
    height = 1000, width = 3000,
    units = "px", res = 300)
par(mfrow = c(1,4), mar = c(4,4,4,0.5))
uniq_col_vec <- c(2,4,3,5)
break_vec <- seq(-max_val-0.15, max_val+0.15, by = 0.1)
...
other_idx <- setdiff(other_idx, de_idx)
hk_idx <- setdiff(hk_idx, c(other_idx, de_idx))

png(paste0("../../../../out/fig/Writeup11g/Writeup11g_sns_layer23_diagnostic_gene.png"),
