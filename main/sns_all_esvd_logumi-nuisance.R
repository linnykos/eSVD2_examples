...
celltype_vec <- c("astpp", "endothelial", "insst", "invip", "layer4", "layer23",
...
for(celltype in celltype_vec){
  ...
  cor_val1 <- stats::cor(x_vec, y_vec)
  xlim <- stats::quantile(x_vec, probs = c(0.01, 1))
  ylim <- stats::quantile(y_vec, probs = c(0.005, 0.995))
  
  p <- length(y_vec)
  cex_vec <- rep(1, p)
  png(paste0("../../../out/fig/main/sns_", celltype, "_scatterplot_logumi-nuisance.png"),
