...
keep_vec <- rep(1, n)
keep_vec[which(regevEpi$Subject == "N110")] <- 0
...
table(regevEpi$Sample_Location, regevEpi$Celltype)
...
for(group in group_vec){
  ...
  ggplot2::ggsave(filename = paste0("../../../../out/fig/Writeup11h/Writeup11h_regev_umap-", group, ".png"),
                  plot1, device = "png", width = 8, height = 5, units = "in")
}

save(regevEpi, date_of_run, session_info,
     file = "../../../../out/Writeup11h/Writeup11h_regevEpi_preprocessed.RData")

##########

table(regevEpi$Celltype, regevEpi$Sample_Location)
idx <- intersect(which(regevEpi$Celltype == "TA 1"), which(regevEpi$Sample_Location == "Epi"))
table(regevEpi$Subject_Disease[idx], regevEpi$Subject[idx])
table(regevEpi$Sample_Health[idx], regevEpi$Subject[idx])
